"""
This file is a part of Internshala-automation Package.
"""
...
class internshala:
    ...
    def get_internship_info(self, url):
        ...
        if self.page.get_by_role("button", name="Already Applied").is_visible():
            ...
            if self.page.locator("#easy_apply_button").is_visible():
                apply_button = self.page.locator("#easy_apply_button")
            elif self.page.locator("#top_easy_apply_button").is_visible():
                apply_button = self.page.locator("#top_easy_apply_button")
            else:
    ...
    # Check for intermediate resume page
    ...
    # Handle assignment questions
        ...
        for x in range(count):
            ...
            if validate_assignment_question:
                ...
                if res["send_to_chatbot"]:        
                    self.answer = GPT.get_assignment_answer(
                        self.profile, self.company, self.about, self.skills, self.que_text, self.is_int_or_job
                    )
                    ...
                    self.page.close()
                    return False  # Make sure this is inside the function
        ...
        # Fill answer in text area
            ans_loc = assignments.nth(x).locator('textarea[id^="text_"]')
            try:
                ans_loc.wait_for(state="visible", timeout=60000)
                ans_loc.fill(self.answer)
            except TimeoutError:
                print("Timeout exceeded while waiting for the answer input field.")

            # ans_loc.fill(self.answer, timeout=60000)

    # Click submit button
        submit_loc = self.page.locator('#submit')
        submit_loc.click()

    # Wait for confirmation modal
        try:
            self.page.wait_for_selector('#similar_job_modal .modal-body .text-heading', state='visible', timeout=10000)
            print('[bold green]Successfully Applied at: [/]', self.page.url)
        except:
            print('[bold red]Application may not have gone through (no confirmation modal).[/]')

    # Store session state if needed
        if hasattr(self, "intern_state_conf"):
            self.page.context.storage_state(path=self.intern_state_conf)
